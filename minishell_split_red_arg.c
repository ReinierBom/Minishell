...
static size_t	len_red_arg(t_cmdl *cmdl, t_pipe *pipe, size_t red, size_t start)
{
	size_t	len;
	size_t	quote;
	...
	len = 0;
	while (quote > 0 || (check_red(pipe->line, start + len) == 0 && space(pipe->line[start + len]) == false && pipe->line[start + len] != '\0'))
	{
		len++;
